...
public class UploadController {
    @Value("${com.shinhan.upload.path}") // C:\\upload
	...
	public ResponseEntity<List<UploadResultDTO>> uploadFile(MultipartFile[] uploadFiles, String mid) {
		...
		for (MultipartFile uploadFile : uploadFiles) {
			...
			try {
				// Member가 있는지 확인
				if (memberRepo.findById(mid).orElse(null) == null) {
					break; // 없으면 for문 빠져나가기
				};
				...
				String thumbnailSaveName = uploadPath + File.separator + folderPath + File.separator + "s_" + uuid + "_"
						+ fileName; // 썸네일 파일 이름은 중간에 s_로 시작하도록
				File thumbnailFile = new File(thumbnailSaveName);
				Thumbnailator.createThumbnail(savePath.toFile(), thumbnailFile, 100, 100); // 썸네일 생성
				resultDTOList.add(new UploadResultDTO(fileName, uuid, folderPath));

				System.out.println(new UploadResultDTO(fileName, uuid, folderPath));

				// ---------- DB에 저장(하나의 멤버가 여러 개의 Profile을 가짐) ----------
				ProfileEntity entity = ProfileEntity.builder().fname(saveName).member(member)
						.currentYn(i == uploadFiles.length - 1 ? true : false).build();
		...
		// ResponseEntity : 데이터와 상태 값을 동시에 전달할 수 있다.
